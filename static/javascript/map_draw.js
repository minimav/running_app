...
function useRoutingBetweenSegments() { return document.getElementById("route-on-click").checked; }

/** Whether to allow multiple runs per data. */
function allowMultipleRunsPerDate() { return document.getElementById("allow-multiple").checked; }

/** Store the current run. */
document.getElementById("submit").onclick = function () {
  const date = document.getElementById("run-date").value
  const distanceMiles = document.getElementById("current-distance").value / 1.609
  ...
  routeData.forEach(data => {
    (data.segmentIds ?? []).forEach(({ segmentId, direction }) => {
      const incrementCount = segmentId !== currentSegmentId || direction !== currentDirection
      if (incrementCount) {
  ...
  routeData.flatMap(data => data.routeFromPrevious ?? data.lineFromPrevious ?? [])
    .forEach(geometry => {
      geometry._latlngs.forEach(({ lat, lng }) => {
        const { jitteredLat, jitteredLng } = jitter(
          { lat, lng, jitterProb: 0.5, maxJitter: 0.000025 }
        )
  ...
  fetch(
    ...
    .then(() => {
      populateAndShowModal({
        ...
        content: `Run on ${date} with ${Object.keys(segmentTraversals).length} unique segments has been stored.`
      })
      reset()
    })
    .catch((err) => populateAndShowModal({
      title: "Submission error",
      content: err
    }))
}
...
function createNodeKey(startNode, endNode) {
  if (startNode < endNode) { return startNode + "_" + endNode }
  else { return endNode + "_" + startNode }
}
  ...
  fetch("/geometry")
    ...
    .then(data => {
      ...
      data.features.forEach(initialSegment => {
        ...
        // within routes
        if (minLengths[nodeKey] === undefined || segment["properties"]["length_m"] < minLengths[nodeKey]) {
...
function zoomToSegment(segmentId) {
  ...
  if (segmentGeometry === undefined) {
    ...
    segmentGeometry = new L.Polyline(
      segmentData[spatialIndexKey]["geometry"]["coordinates"].map(x => { return [x[1], x[0]] }),
    );
  }
...
function removeSegmentsNoLongerInView(spatialIndexKeysOfSegmentsInNewView) {
  ...
  spatialIndexKeysOfSegmentsInCurrentView.forEach(x => {
    if (!spatialIndexKeysOfSegmentsInNewView.has(x)) { spatialIndexKeysOfSegmentsToRemove.add(x) }
  })
  ...
  console.debug(spatialIndexKeysOfSegmentsToRemove.size + " segments to remove")
...
function filterToNewSpatialIndexKeys(spatialIndexKeysOfSegmentsInNewView) {
  ...
  spatialIndexKeysOfSegmentsInNewView.forEach(key => {
    ...
    if (!spatialIndexKeysOfSegmentsInCurrentView.has(key) || ignoredSegments.has(segmentId)) {
...
function showSegments() {
  ...
  if (map.getZoom() <= 13) {
    try {
      ...
      removeSegments(
        new Set([...Array(spatialIndex.numItems).keys()])
      )
  ...
  let spatialIndexKeysOfSegmentsInNewView = getSpatialIndexKeysOfSegmentsInView(map.getBounds())
  console.debug(spatialIndexKeysOfSegmentsInNewView.size + " segments in new view.")
  removeSegmentsNoLongerInView(spatialIndexKeysOfSegmentsInNewView)
  let spatialIndexKeysOfNewSegmentsInView = filterToNewSpatialIndexKeys(spatialIndexKeysOfSegmentsInNewView)
...
const updateRunSpeed = (distanceMiles) => {
  ...
  document.getElementById("current-speed").value = (distanceMiles > 0) && (durationMinutes > 0)
...
function updateLengthKm(lengthKm) {
  ...
  document.getElementById("current-distance").value = Math.max(0.0, newLengthKm).toFixed(3)
...
document.querySelector("#duration").addEventListener("change", () => {
  try {
    const distanceMiles = 1.609 * parseFloat(document.getElementById("current-distance").value)
...
function removeCurrentlySegmentCollection() {
  let keys = Array.from(ignoredSegments).map(segmentId => segmentIdToSpatialIndexKey[segmentId])
  removeSegments(keys)
}
...
function undo() {
  if (routeData.length > 0) {
    ...
    removeGeometries(
      previousRouteData,
      ["clickedPoint", "snappedPoint", "snapLine", "lineFromPrevious", "routeFromPrevious", "distanceTooltip"]
    )
    updateLengthKm(-previousRouteData.distanceKm ?? 0.0)

    if (routeData.length > 0) {
      const newPreviousRouteData = routeData[routeData.length - 1]
      const props = routeData.length > 1 ? { fillColor: '#dc3545', radius: 8 } : { radius: 8 }
...
const removeUploadedGeometries = () => {
  [uploadedRunPolyline, uploadedStartPoint, uploadedEndPoint].forEach(geom => {
    try { map.removeLayer(geom) } catch { }
  })
}
...
const redo = () => {
  ...
  const previousLatLng = [routeToRedo.clickedPoint._latlng.lat, routeToRedo.clickedPoint._latlng.lng]
...
function findNearestSegmentAndPointOnIt({ lat, lng }, spatialIndexKeys) {
  ...
  spatialIndexKeys.forEach(key => {
    ...
    coordinates.forEach(([otherLng, otherLat], index) => {
      const distanceMetres = haversineDistanceMetres([lat, lng], [otherLat, otherLng])

      if (index > 0) {
        const [previousLng, previousLat] = coordinates[index - 1]
        cumulativeDistanceMetres += haversineDistanceMetres([previousLat, previousLng], [otherLat, otherLng])
      }
...
function snapToNetwork(event) {
  ...
  const clickedPoint = new L.CircleMarker(
    event.latlng,
    { radius: 8, fillOpacity: 1.0, fillColor: fillColor, color: 'blue', pane: 'points' }
  )
  ...
  let latBuffer = 0.001, lngBuffer = 0.001, factor = 0
  let nearbySpatialIndexKeys = []
  while ((nearbySpatialIndexKeys.length === 0) && (factor < 5)) {
    nearbySpatialIndexKeys = getSegmentsNearbyToLatLng(lat, lng, latBuffer * Math.pow(2, factor), lngBuffer * Math.pow(2, factor))
    factor += 1
  }
  ...
  if (nearbySpatialIndexKeys.length === 0 && routing) {
    if (routing) {
      populateAndShowModal({
        ...
        content: `No segments found within buffer around (${lat.toFixed(3)}, ${lng.toFixed(3)}). Try closer `
    ...
    snapLine = new L.Polyline(
      [[snap.point.lat, snap.point.lng], [lat, lng]],
      {
        color: '#fd7e14',
        weight: 3,
        opacity: 1.0,
  ...
  routeData.slice(1).forEach(data => {
    ...
    const props = data.distanceTooltip === undefined ? { fillColor: 'blue', radius: 4 } : { fillColor: 'blue' }
  ...
  if (routing && previousRouteData.snappedPoint !== undefined) {
    ...
    if (map.hasLayer(previousRouteData.lineFromPrevious)) {
      ...
      const previousAgainPoint = map.hasLayer(previousAgainRouteData.snappedPoint)
      ...
      const overrideStraightLineCoords = [
        ...
        [previousRouteData.snappedPoint._latlng.lat, previousRouteData.snappedPoint._latlng.lng],
      ]
      ...
      removeGeometries(previousRouteData, ["clickedPoint", "lineFromPrevious", "distanceTooltip"])

      const newPreviousDistanceKm = haversineDistanceMetres(...overrideStraightLineCoords) / 1000
    ...
    const previousSegment = segmentData[previousRouteData.snapInfo.spatialIndexKey]["properties"]
    ...
    const payload = {
      ...
      body: JSON.stringify({
        ...
        from_segment_distance_along_segment_metres: previousRouteData.snapInfo.distanceAlongSegmentMetres,
    ...
    fetch('/route', payload)
      ...
      .then(data => {
        ...
        data.route.forEach((routeSegment, index) => {
          const forwardsDirection = routeSegment.start_distance_metres < routeSegment.end_distance_metres

          let spatialIndexKey, allCoords
          try {
            spatialIndexKey = segmentIdToSpatialIndexKey[routeSegment.segment_id]
            allCoords = switchCoords(segmentData[spatialIndexKey]["geometry"]["coordinates"])
            routeSegmentIds.push({ segmentId: routeSegment.segment_id, direction: forwardsDirection })
          } catch (err) {
            const nodeKey = createNodeKey(routeSegment.start_node, routeSegment.end_node)
            spatialIndexKey = nodesToSpatialIndex[nodeKey]
            allCoords = switchCoords(segmentData[spatialIndexKey]["geometry"]["coordinates"])
            const segmentId = segmentData[spatialIndexKey].properties.segment_id
          ...
          if ((index === 0) && (data.route.length === 1)) {
            ...
            const slicedCoords = forwardsDirection
              ? allCoords.slice(previousRouteData.snapInfo.geometryIndex, snap.geometryIndex + 1)
              : allCoords.slice(snap.geometryIndex, previousRouteData.snapInfo.geometryIndex + 1).reverse()

            routeCoordinates.push(...slicedCoords)
            distanceKm += Math.abs(routeSegment.end_distance_metres - routeSegment.start_distance_metres) / 1000
          } else if (index === 0) {
            // start segment case
            const slicedCoords = routeSegment.end_distance_metres === 0.0
              ? allCoords.slice(0, previousRouteData.snapInfo.geometryIndex + 1).reverse()
            ...
            // end segment case
            const slicedCoords = routeSegment.start_distance_metres === 0.0
        ...
        const routeLine = new L.Polyline(
          routeCoordinates,
          {
            color: 'blue',
            weight: 3,
        ...
        if ((oldLengthKm.toFixed(0) !== newLengthKm.toFixed(0)) & (newLengthKm - newLengthKm.toFixed(0) < 0.2)) {
          tooltip = createDistanceTooltip(snap.point.lat, snap.point.lng, `${newLengthKm.toFixed(0)}`)
          tooltip.addTo(map)
        }
    ...
    const straightLine = new L.Polyline(
      [[previousPoint._latlng.lat, previousPoint._latlng.lng], [lat, lng]],
      {
        color: 'blue',
        weight: 3,
        opacity: 1.0,
    ...
    const distanceKm = haversineDistanceMetres(
      [previousPoint._latlng.lat, previousPoint._latlng.lng], [lat, lng]
    ) / 1000
...
const upload = () => {
  ...
  fetch("/upload_run", payload)
    ...
    .then(data => {
      ...
      const runLatLngs = parsedRunWkt.components.map(p => { return L.latLng([p["x"], p["y"]]); })

      uploadedRunPolyline = new L.Polyline(runLatLngs, { color: "black", pane: "uploads" })
      uploadedStartPoint = new L.CircleMarker(
        runLatLngs[0],
        { radius: 8, fillOpacity: 1.0, fillColor: '#198754', color: 'black', pane: "upload-points" }
      )
      uploadedEndPoint = new L.CircleMarker(
        runLatLngs[runLatLngs.length - 1],
        { radius: 8, fillOpacity: 1.0, fillColor: '#dc3545', color: 'black', pane: "upload-points" }
      )
...
document.addEventListener("DOMContentLoaded", function (event) {
  ...
  map.on('zoomend', function () { showSegments() })
  map.on('dragend', function () { showSegments() })
  map.on('moveend', function () { showSegments() })
