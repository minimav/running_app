...
const submit = () => {
  ...
  if (polygonAreaKmSquared > maxPolygonAreaKmSquared) {
    populateAndShowModal({
      ...
      content: `Polygon must have area < ${maxPolygonAreaKmSquared}km${'2'.sup()}, area was ${polygonAreaKmSquared.toFixed(3)}km${'2'.sup()}.`
    })
    return
  }
  ...
  populateAndShowModal({
    ...
    content: "Background task running to retrieve graph and geometry for chosen polygon."
  })
    ...
    .then(response => {
      ...
      var timer = setInterval(function () {
        ...
        fetch("/current_user_areas")
          ...
          .then(data => {
            const currentAreasWithArtifacts = new Set(data.map(d => d.area_name))

            if (currentAreasWithArtifacts.has(runAreaName)) {
              document.getElementById("create-run-area-submit").disabled = false
              clearInterval(timer)
              populateAndShowModal({
                title: "Data retrieved",
                content: 'Background task finished, visit <a href="/">Home</a> and switch area to start logging runs.'
              })
            }
          })
      }, 10000)
...
document.addEventListener("DOMContentLoaded", function (event) {
  ...
  document.getElementById("create-run-area-submit").addEventListener("click", submit)
